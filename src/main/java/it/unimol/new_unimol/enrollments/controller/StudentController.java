...
public class StudentController {
    ...
    public ResponseEntity<?> selfEnrollToCourse(
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.CREATED).body("Iscrizione creata con successo" + enrollment);
    ...
    public ResponseEntity<?> checkSelfServiceAvailability(
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.OK).body("Verifica completata" + canEnroll);
        } catch (Exception e) {
    ...
    public ResponseEntity<?> requestEnrollmentToCourse(
        ...
        try {
            ...
            String requestNote;
            if(requestBody != null) {
                requestNote = requestBody.get("requestNote");
            } else {
                requestNote = null;
            }

            EnrollmentRequestDto request = studentService.createEnrollmentRequest(courseId, studentId, requestNote);
            return ResponseEntity.status(HttpStatus.CREATED).body("Richiesta creata con successo" + request);
        } catch (IllegalArgumentException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(e.getMessage());
        } catch (IllegalStateException e) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body(e.getMessage());
